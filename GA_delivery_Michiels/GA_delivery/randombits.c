...
unsigned short USHRTran(void){ register unsigned char i;
    ...
    for(i=0; i < USHRT_WIDTH; i++){
        unsigned char y=random_bit();
        printf("\neerste bit %hhu",y);
        if(y) { base = oneU; break; }
    } 
    for(i++; i < USHRT_WIDTH; i++){ 
        base = base << 1;
        unsigned char x=random_bit();
        printf("| %hhu",x);
        if(x) base = base | oneU;
    }
    return base;
...
unsigned long ULONGran(unsigned char j){ register unsigned char i;
    unsigned long oneU = 1U, base = 0U;
    unsigned char s = ULONG_WIDTH -j;
    for(i=s; i < ULONG_WIDTH; i++) {
        if(random_bit()) { base = oneU; break; }
    }
    for(i++; i < ULONG_WIDTH; i++){ 
        base = base << 1;
